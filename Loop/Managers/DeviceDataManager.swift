...
extension DeviceDataManager {
    func enactBolus(units: Double, activationSource: DoseActivationSource, completion: @escaping (_ error: Error?) -> Void = { _ in }) {
        ...
        pumpManager.enactBolus(units: units, activationSource: activationSource) { (error) in
            if let error = error {
                ...
                default:
                    ...
                    if activationSource != .recommendationAutomatic {
                        NotificationManager.sendBolusFailureNotification(for: error, units: units, at: Date(), activationSource: activationSource)
                    }
                }
