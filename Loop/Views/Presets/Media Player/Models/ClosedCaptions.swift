...
struct ClosedCaptions: Equatable, Hashable, RawRepresentable {
    
    var fragments: [ClosedCaptionFragment]
    
    var rawValue: String {
        fragments.map(\.rawValue).joined(separator: "\n\n")
    }
    
    init(fragments: [ClosedCaptionFragment]) {
    ...
    func currentFragment(at timecode: TimeInterval) -> ClosedCaptionFragment? {
