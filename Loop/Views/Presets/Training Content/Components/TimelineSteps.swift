...
struct TimelineBuilder {
    static func buildBlock(_ components: TimelineStep...) -> [TimelineStep] {
        components
    }
}

protocol TimelineStyle {
...
struct DefaultTimelineStyle: TimelineStyle {}

extension TimelineStyle where Self == DefaultTimelineStyle {
...
struct TimelineStep {
    ...
    init(symbol: Image, symbolInset: Double = 0, title: Text, subtitle: Text) {
        self.symbol = symbol
...
struct Timeline: View {
    ...
    func style(_ style: TimelineStyle) -> Timeline {
        var copy = self
        copy.style = style
        return copy
    }
